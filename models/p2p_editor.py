...
class P2PEditor:
    def __init__(self, method_list, device, model_path,args) -> None:
        self.device=device
        self.method_list=method_list
        self.num_ddim_steps=args.num_steps
        self.args = args
        # init model
        ...
        self.scheduler = DDIMScheduler(beta_start=0.00085,
                                    ...
                                    clip_sample=False,
                                    set_alpha_to_one=False)
        self.ldm_stable = StableDiffusionPipeline.from_pretrained(model_path, scheduler=self.scheduler).to(device)
    ...
    def __call__(self, 
        ...
        if edit_method=="ddim+p2p":
            return self.edit_image_ddim(image_path, prompt_src, prompt_tar, guidance_scale=guidance_scale, 
                                        ...
                                        blend_word=blend_word, eq_params=eq_params, is_replace_controller=is_replace_controller,height=self.args.height,width=self.args.width)
    ...
    def edit_image_ddim(
        ...
        image_gt = load_512(image_path,height=height,width=width)
